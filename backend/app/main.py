...
async def diagnostic_middleware(request: Request, call_next):
    ...
    if request.url.path.startswith("/api"):
        try:
            ...
            # If DB fails, return details
            return JSONResponse(
                status_code=500,
                content={"detail": f"Database Connection Error: {str(db_err)}", "type": "db_error"}
            )
            
    # Now that DB is checked, handle ping
    if request.url.path == "/api/ping":
        return JSONResponse({"status": "alive", "db": "connected", "startup_error": STARTUP_ERROR})
...
async def root():
    try:
        ...
        return HTMLResponse(content="<h1>Backend is running</h1><p>Visit /api/ping to check DB.</p>")
    except Exception as e:
